...
| **Data Integrity** | Every 4 KB chunk is validated using MD5 checksums to detect and reject corrupted packets. |
...
**Requires Python 3.12+.** Clone or extract the repository files to your local machine before proceeding.
...
> **Note:** When using `uv`, you do not need to manually activate the virtual environment. Use the `uv run` command as shown in the [Usage](#-usage-instructions) section below.
...
- **Using `uv` (Option A)** — `uv run` automatically uses the correct virtual environment:
  ```bash
  uv run app.py
  ```

- **Using `pip` (Option B)** — ensure your virtual environment is activated, then run:
  ```bash
  python app.py
  ```
...
2. **Configuration:**
   - **Host** — Enter the server's IP address (use `127.0.0.1` for local testing).
...
4. **Downloading:**
   ...
   - You can pause/resume the transfer at any time. The UI displays real-time throughput in MB/s.
...
Client                          Server (Port 9000)
  ...
  |                                     |  Binds to ephemeral port (e.g. 54321)
  |◄─── "Connect to port 54321" ────────|
  |                                     |
  |──── Data stream (port 54321) ──────►|  Isolated transfer session
  |◄─── ACK / NACK ────────────────────|
```

1. **Main Listener (Port 9000)** — Continuously polls for incoming `REQUEST` or `UPLOAD` commands. On receipt, validates the request and spawns a daemon thread.
2. **Dynamic Session Ports** — The spawned thread binds to a new, randomly assigned ephemeral port and replies to the client with this address.
3. **Isolated Transfer Streams** — The client switches its target to the session port, ensuring that large file chunks never clog the main listener and keeping concurrency high.
...
| File | Description |
|---|---|
| `app.py` | Main GUI application containing the `FileTransferServer` and `FileTransferClient` protocol adapters. |
| `encryption.py` | Manages the Fernet symmetric encryption lifecycle and keys. |
| `utils.py` | Core constants (`CHUNK_SIZE`) and MD5 checksum hashing functions. |
| `pyproject.toml` | Project metadata and dependency declarations for `uv`. |
| `uv.lock` | Lockfile for fully reproducible `uv` installs. |
...
- If the GUI fails to launch, try installing dependencies manually via `pip`.
- Use `127.0.0.1` as the host when testing client and server on the same machine.
